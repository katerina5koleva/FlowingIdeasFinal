...
public partial class IdeasDataViewer : Form
{
	...
	private void UpdateGrid(int userId)
	{
		...
		if (dataGridView1.Columns["IdeaTypeId"].HeaderText != "Idea Type")
		{
	...
	private void FilterAndDisplayIdeas()
	{
		...
		{
			filteredIdeas.AddRange(ideaBusinessLogic.GetIdeasByType(userId, IdeaType.Philosopical));
		}
		if (artisticIdeaCheckBox.Checked)
		{
			filteredIdeas.AddRange(ideaBusinessLogic.GetIdeasByType(userId, IdeaType.Artistic));
		}
		if (workIdeaCheckBox.Checked)
		{
			filteredIdeas.AddRange(ideaBusinessLogic.GetIdeasByType(userId, IdeaType.Work));
		}
	...
	private void IdeasDataViewer_Load(object sender, EventArgs e)
	{
